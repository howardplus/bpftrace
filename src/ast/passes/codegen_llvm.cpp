...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    */
    ...
    FunctionType *callback_type = FunctionType::get(b_.getInt64Ty(), callback_args, false);
    ...
    if (call.func == "clear") {
      ...
      bool is_per_cpu = (map_info->second.bpf_type == BPF_MAP_TYPE_PERCPU_ARRAY || map_info->second.bpf_type == BPF_MAP_TYPE_LRU_PERCPU_HASH);
      if (is_per_cpu) {
...
void CodegenLLVM::createPrintMapCall(Call &call)
{
  ...
  b_.CreateOutput(buf, getStructSize(print_struct), call.loc, true);
  b_.CreateLifetimeEnd(buf);
}
